...
class MILPSolver:
    ...
    def solve(self):
        ...
        while self.node_queue:
            ...
            lp_solver = SimplexSolver(copy.deepcopy(current_problem))
            solution = lp_solver.solve()
            
            if solution["status"] != "Optimal":
                print(f"Nó podado. Status do LP: {solution['status']}")
                continue
            ...
            print(f"Ramificando na variável '{branch_var_name}' com valor {branch_var_value:.4f}")
            
            problem_down = self._add_constraint_to_problem(current_problem, branch_var_name, "<=", np.floor(branch_var_value))
            problem_up = self._add_constraint_to_problem(current_problem, branch_var_name, ">=", np.floor(branch_var_value) + 1)
            
            self.node_queue.extend([problem_up, problem_down])
        ...
        if self.best_integer_solution:
             ...
             recalculated_obj = 0
             for var, val in solution_to_print["variables"].items():
                if val > 1e-6: # Apenas para variáveis com valor
                    idx = self.root_problem.variable_names.index(var)
                    recalculated_obj += self.root_problem.objective_coeffs[idx] * val
             print(f"Status: {solution_to_print['status']}")
             print(f"Valor da Função Objetivo: {recalculated_obj:.4f}")
             print("\nValores das Variáveis:")
             for var, value in solution_to_print["variables"].items():
                 if value > 1e-6:
    ...
    def _is_integer_feasible(self, solution_vars: dict, problem: Problem) -> bool:
        for var_idx in problem.integer_variables:
            ...
            is_binary = (self.root_problem.lower_bounds[var_idx] == 0 and self.root_problem.upper_bounds[var_idx] == 1)
            if is_binary and not (abs(value - 0) < 1e-6 or abs(value - 1) < 1e-6): return False
        return True
    ...
    def _find_branching_variable(self, solution_vars: dict, problem: Problem) -> str:
        for var_idx in problem.integer_variables:
            ...
            is_binary = (self.root_problem.lower_bounds[var_idx] == 0 and self.root_problem.upper_bounds[var_idx] == 1)
            if is_binary and not (abs(value - 0) < 1e-6 or abs(value - 1) < 1e-6): return var_name
        return None