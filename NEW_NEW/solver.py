...
from heuristics import run_feasibility_pump, run_rins
from presolve import run_bound_strengthening
...
class MILPSolver:
    ...
    def _solve_lp_relaxation(self, node: Node) -> None:
        """
        [CORRIGIDO] Resolve a relaxação de LP e SEMPRE armazena a solução completa
        para garantir consistência de dados em todo o solver.
        """
        node_model = self.model.copy()
        ...
        if status == GRB.OPTIMAL:
            ...
            # Armazena sempre a solução completa no nó
    ...
    def solve(self, problem_path: str):
        ...
        # FASE 2: RELAXAÇÃO E CORTES NA RAIZ (OPCIONAL)
        for v in self.model.getVars():
            if v.VType != GRB.CONTINUOUS: v.VType = GRB.CONTINUOUS
        self.model.update()

        root_node = Node(parent_id=None, depth=0, local_bounds={})
        self._solve_lp_relaxation(root_node)
        ...
        if self.config.get('use_presolve', False):
            run_bound_strengthening(self.model, self.original_vars)
        ...
        # FASE 1: HEURÍSTICA PRIMAL (OPCIONAL)
        if self.config.get('use_heuristics', False):
            # A chamada à heurística usa o modelo MILP original
            heuristic_result = run_feasibility_pump(self.model, self.original_vars)
            if heuristic_result:
                # O solver valida a solução retornada pela heurística
                temp_model = self.model.copy()
                solution_vars = heuristic_result['solution']
                for v_name, v_val in solution_vars.items():
                    var = temp_model.getVarByName(v_name)
                    var.lb = v_val
                    var.ub = v_val
                temp_model.optimize()
                if temp_model.Status == GRB.OPTIMAL:
                    self.incumbent_value = temp_model.ObjVal
                    self.incumbent_solution = solution_vars
                    print(f"INFO: [Solver] Heurística encontrou solução viável com objetivo: {self.incumbent_value:.4f}")
        ...
        while not self.tree.is_empty():
            ...
            if current_node.is_integer:
                if current_node.lp_objective_value < self.incumbent_value:
            ...
            branch_var_name = self.branching_strategy.select_variable(current_node, self.model)
            ...
            self._solve_lp_relaxation(child1)
            ...
            self._solve_lp_relaxation(child2)
